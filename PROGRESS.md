...
Detailed running notes so work can resume after a context reset. Last updated: **2026-06-27**.
...
- **Web already has a complete Printful POD** (`foto-mozaik/lib/print-products.ts`, `/api/print/*`). Decision: **leave Printful untouched**, build a **parallel Prodigi** stack under `/api/print/prodigi/*` + `prodigi_orders` table. Mobile uses Prodigi. **As of 2026-06-18 the web UI also uses Prodigi** (Printful code left in place but no longer referenced from the studio — see "Web Prodigi flow" below).
...
#### Web Prodigi flow — replaces Printful in the web UI (2026-06-18)
- **Goal (user):** make the **web** print flow use Prodigi, mirroring the mobile flow, to fully replace Printful. **Scope chosen:** *switch the web UI now, leave Printful code in place (dead) for a later deletion pass*; **add a web "My orders" page** like mobile.
- **No backend changes** — web reuses the same `/api/print/prodigi/{products,checkout,fulfill,orders,webhook}` + `lib/prodigi*.ts` already built for mobile (same Creem/Prodigi env, `PRODIGI_CREEM_*`, `getMaxResolutionPrint()` render, 30-day asset tokens, etc.). The Prodigi model is leaner than Printful: **checkout stores the slim design** (`plan`/`tileUrls`/`baseUrl`/`cropRect`/`attributes`); the print mosaic is **rendered server-side in fulfill after payment, no token**. The old web Printful "render high-res + proof BEFORE paying" step is gone.
- **New files (foto-mozaik):**
  - `lib/prodigi-ui.ts` — client presentation catalog mirroring mobile `print_catalog.dart`: `FRAME_COLORS`/`CANVAS_WRAPS`/`METAL_FINISHES`, `TYPE_LABEL`/`TYPE_BLURB`, `attrKeyForType`/`attrChoicesForType`/`defaultAttributes`, and `optionSummary()` (same wording as mobile: "Image wrap"/"Black frame"/"Lustre finish").
  - `components/print/prodigi-cropper.tsx` — fixed-aspect crop frame (drag-pan + wheel/slider zoom) that emits a **normalised `cropRect {left,top,width,height}` (0..1)** — exactly what fulfill bakes in. `centerCrop(w,h,aspect)` helper for defaults. (No rotation at crop time; landscape reuses portrait SKU upright, `rotate:false` everywhere — consistent with the mobile decision.)
  - `components/print/prodigi-order-modal.tsx` — orchestrator mirroring mobile `wall_art → crop → address → review → pay → processing`: steps **product** (type cards → orientation auto-picked from mosaic aspect via `orientationForAspect` → frame colour/edge/finish, live cropper preview with frame-colour border) → **address** (`AddressForm`, name+email **prefilled from `/api/user`**) → **review** (cropped framed thumbnail + option summary + ship-to; "no tokens used for prints") → **pay** (Creem popup) → **processing** (calls fulfill) → **done** ("View my orders").
  - `app/orders/page.tsx` — web **My orders** (client page): thumbnail, status badge, `optionSummary`, total, **tracking number + Copy + Track link** (when shipped), and **Resume processing** for paid-but-unsubmitted orders (same `isResumable` rule: `!prodigiOrderId && status ∈ {paid,uploading,submitted,failed}` → `POST /api/print/prodigi/fulfill`). 401 → "please sign in".
- **Wiring:** `components/mosaic-studio.tsx` swapped `PrintOrderModal` → `ProdigiOrderModal` (props identical — the studio already builds `mosaicPlan = JSON {plan,tileUrls,baseUrl}` + `mosaicPreviewUrl`); added a "My orders" footer link (→ `/orders`).
- **Fulfilment trigger (web):** checkout `success_url = /print-success?prodigi=<id>` (popup). `app/print-success/page.tsx` gained a **`?prodigi=` branch**: marks the order paid, shows an EUR confirmation, links to `/orders`, and renders `<PrintSuccessBroadcast/>`. The opener modal hears the broadcast (channel `creem-payment-print` / LS `creem-payment-print-success`) and **calls `/api/print/prodigi/fulfill` from the authenticated browser** (cookies). If that's missed (tab closed), the order stays `paid` and is **resumable from My orders** — same safety net as the app. (`fulfill` is session-owner-authed + idempotent.)
- **Popup parity with the mobile wall-art screen (2026-06-18, follow-up):** reworked the modal so it looks like `wall_art_screen.dart` — realistic **mockup fills the top**, compact controls below (type cards → orientation → frame colour/edge/finish), an **"Actual size" + "Framing"** button row, then the Order CTA. **Fit-screen:** modal is now a fixed-height flex column (`h-[92vh] max-h-[940px]`, header pinned, mockup `flex-1` shrinks, controls in a `shrink-0` strip) — no whole-page scroll; framing/address/review are sub-views in the same frame.
  - `components/print/prodigi-mockup.tsx` — **canvas port of the mobile `MockupPainter`**: wall photo (`public/print-wall.jpg`, copied from `imagia-mobile/assets/wall.jpg`) cover-fit + dim/vignette/floor, then **framed** (colour frame + shadow + inner line), **canvas** (3D gallery-wrap — image continues onto right/bottom edges via an **affine `drawImageQuad`** + weave), **metal** (float shadow + glossy diagonal sheen). No `crossOrigin` on the mosaic img (draw-only, avoids CORS load failures on cross-origin preview URLs).
  - `components/print/prodigi-actual-size.tsx` — **"Actual size"** pannable view at ~true printed scale via `longEdgeCm(type,orientation)` (added to `lib/prodigi-ui.ts`, mirrors mobile `printSpec`; `PX_PER_CM = 96/2.54`). NB it scales the **preview** image (tiles soften when enlarged; exact size is monitor-dependent → labeled "approximate"), whereas mobile re-renders plan tiles sharply — wire to the full-res render later if pixel-sharp is wanted.
  - Modal steps are now `product | crop | actualsize | address | review | processing | done` (crop = `ProdigiCropper` reached via "Framing"). `tsc`+`eslint` clean.
- **Verified:** `tsc --noEmit` + `eslint` clean on all changed files (did not run full `next build`). No leftover `PrintOrderModal` refs in the studio.
- **Still to do (web POD):** test live/sandbox on web; the Printful **deletion pass** (`/api/print/{checkout,render,mockup,products,fulfill,webhook}`, `lib/printful.ts`, `lib/print-products.ts`, Printful-only `components/print/*` — watch admin/db refs + in-flight Printful orders); consider a more prominent My-orders entry point (currently studio footer).
...
## Post-launch fixes (app is LIVE on the App Store)

### Export crash: "Cannot convert argument to a ByteString … character at index N has a value of >255" (2026-06-23)
- **Reported by a user during export.** Root cause: a **non-Latin1 character** (e.g. `Đ` U+0110 = 272, common in HR/RS names) in the user's **name** or **photo filename** was written into an **HTTP header** (header values must be a ByteString, 0–255) → `fetch()`/`Headers` throws; the message is surfaced to the app's Export screen.
- Two server touchpoints in `foto-mozaik` (both fixed):
  1. `app/api/render/route.ts` sent `X-User-Name: session.user.name` (+ `X-User-Email`) to the Cloud Run render service — the **primary culprit**. Added `headerSafe()` (strips chars >255 + CR/LF) and applied it to both headers. Also `headerSafe`'d the `fileName` at intake (mobile sends `"<photo>-mosaic.jpg"`; flows into the blob path **and** the Cloud Run service which sets it as a header).
  2. `app/api/download/[token]/route.ts` put the filename straight into `Content-Disposition` → same crash at download. Now uses **RFC 5987** (`filename="ascii"; filename*=UTF-8''…`) so unicode names work.
- **Fix is 100% server-side → deploying `foto-mozaik` to Vercel fixes every already-installed app instantly; NO Cloud Run redeploy and NO App Store resubmission needed.** Cosmetic only: a download from a `Đ`-named photo has those letters stripped from the *filename* (e.g. `_orde-mosaic.jpg`); the mosaic image is unaffected. `tsc`/`eslint` clean.

## Text Mosaic mode (web-only, foto-mozaik) (2026-06-27)

A new **web-only** mosaic mode: the picture is rebuilt out of the user's **words**, not photos — a "typographic mosaic / word-portrait". No mobile changes. Built by **reusing the existing engine** (a text tile is just a generated image), so preview (`drawAtlas`) + export (`plan + tileUrls` → `/api/render`) work unchanged.

**How it works (the trick):** black text on white paper → perceived gray comes from **ink density** (how much of a cell the text fills). The base photo is converted to **grayscale + contrast** and matched by brightness; each cell gets a word sized/weighted to that brightness.

**Engine (`foto-mozaik/lib/mosaic/`):**
- `types.ts`: `MosaicMode` gains `"text"`; `MosaicSettings` gains `textContrast` (0..1 grayscale contrast push), `textInvert`, `textThreshold` (hard 2-color cutoff instead of the S-curve — `textContrast` becomes the cutoff point, clamped 0.1–0.9), `textOrientation` (`square|portrait|landscape|original`). All threaded through `DEFAULT_SETTINGS` + `sanitizeSettings` (which **whitelists** modes + strips unknown fields — must add there or they're dropped).
- `analyze.ts`: `preprocessPixels` gained grayscale → contrast S-curve → invert (new `PreprocessOptions` fields).
- `browser.ts` `buildBrowserMosaicPlan`: in text mode the base is analyzed **grayscale** (faces skipped); `textOrientation` maps to the cell layout — `square/portrait/landscape` → matching layout, **`original` → `blocks`** (2×2/2×3/3×2 cells line up with mixed-aspect tiles); `aspectWeight` bumped to **0.6** so text tiles land in cells of their own shape (no crop/stretch). `getOrCreateAnalyzer` cache key includes the new preprocess opts.

**Text-tile generator (`foto-mozaik/lib/text-mosaic/text-tile-generator.ts`):**
- `parseTexts(raw, uppercase)` — one phrase per line, trimmed + de-duped.
- `generateTextTiles(texts, {uppercase, orientation, tileSize}, onProgress)` → **5 darkness variants per phrase** (`LEVELS`), light→dark. **Critical insight (2026-06-27):** black text is sparse ink, so even a big bold word is ~60% white — earlier designs (one-tile-per-text, or fill-cap+stroke) couldn't get dark, so the photo's whole dark range collapsed onto light-ish tiles → flat/"image not visible". Current `LEVELS` = **packed vs sparse**: light levels draw a **single sparse word** (lots of white) for highlights; dark levels **PACK the cell with the phrase repeated + word-wrapped at a small font** (`fill: true`, smaller `fontScale` = denser = darker) → genuine white→near-black palette. Letters keep **natural proportions (no squeeze)** — only repeats + font size change. Tiles run through the **existing `createLightweightTileAsset`** (full descriptor) so the matcher/preview/export treat them like photo tiles. Orientation aspects: square 360², landscape 360×240, portrait 240×360; **`original`** picks a per-phrase aspect (≥4 words→portrait, ≤6-char single word→landscape, else square). (Storage = texts × 5; `clearTiles()` deletes the old set on regenerate.)
- **Phrase persistence:** the entered phrases survive reload two ways — (1) **localStorage** (`imagia.textMosaic.input`, lazy-initialized so no clobber; also stores the uppercase toggle; offline/pre-generate fallback) and (2) **server, per project** — new `projects.texts` column (`db/schema.ts`), returned by `GET /api/projects/[id]`, accepted by `PUT`, sent by `saveSessionNow` (`texts: textInput`, autosaves on text/tile/settings change), restored in `loadProject`. **Needs `pnpm db:push`** to add the column.
- **"Inverted" report was the Invert toggle**, not a bug — the grayscale/threshold→brightness→tile mapping is correct (dark region → dense/dark tile). For a portrait on a light background (e.g. the Joe Strummer test) leave Invert **off**.
- **Vertical center-seam fix:** it was a **display artifact** (visible only zoomed out, not in the loupe → the data is fine). The displayed preview canvas was rendered at CSS px and the browser re-sampled it → midline seam. `mosaic-preview.tsx` now renders the canvas at **devicePixelRatio** (backing) and displays at CSS size (no re-sample, also crisper). NB the `COLOR_NUDGE_ALPHA = 0` in `render.ts` means that overlay was already a no-op (a red herring while chasing this); also added an **average-color fallback** in `renderPreviewToCanvas` so a missing/undecoded tile fills its cell instead of leaving a transparent (dark) hole.
- **Preview color-nudge flag:** `renderPreviewToCanvas` (`lib/mosaic/render.ts`) gained an `options.colorNudge` flag; `MosaicPreview` gained a `colorNudge` prop; the studio passes `colorNudge={settings.mosaicMode !== "text"}` (keeps text crisp / avoids any future grey wash). NB this did NOT fix the seam — `COLOR_NUDGE_ALPHA` is currently `0` so the nudge never draws anyway. The seam fix was the DPR change above.

**UI:**
- `settings-panel.tsx`: "Text mosaic — words, not photos" option (sets legible cell size + `tintStrength:0`; restores tint on leaving).
- `components/text-mosaic-panel.tsx` (new): textarea (one phrase/line), **Orientation** selector (Square/Portrait/Landscape/**Mixed**), Uppercase, Invert, Contrast slider, **Generate** button + progress.
- `upload-panel.tsx`: `textMode` prop hides the photo-tile uploader (keeps base-photo upload) + shows a hint.
- `mosaic-studio.tsx`: text state + `handleGenerateTextTiles(orientationOverride?)` + `handleTextOrientationChange` (auto-regenerates at the new aspect if tiles exist); `textMosaicPanelEl` wired into **both** desktop and mobile layouts.

**Storage:** regenerating replaces tiles and calls `uploadManager.clearTiles()`, which **deletes the previous tiles' blobs** via `/api/tiles/delete` — so switching texts/orientation leaves no orphan blobs. One set of tiles kept (no 3× pre-generation of orientations).

**Status:** `tsc --noEmit` + `eslint` clean on all new/changed files (only pre-existing `any`/`prefer-const` lints remain in the big studio file; none added). **NOT browser-tested yet** — needs a visual pass. Known tuning levers if results look flat: the **Contrast** slider, **density** (bigger cells = more legible), text-length variety (one tile per text means tonal range depends on phrase-length spread), and the simple **Mixed** aspect heuristic. Possible follow-up: optional "more tonal range" toggle (2–3 levels per text) if uniform-length inputs read flat; a condensed display font for darker fills.
